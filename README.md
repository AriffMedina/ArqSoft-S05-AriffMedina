...
## 🔄 Rama `CI/CD` — Integración Continua con xUnit + GitHub Actions

Esta rama agrega un pipeline de **Integración Continua (CI)** al proyecto, además de una suite inicial de pruebas unitarias con **xUnit**. El objetivo es que cada `push` compile el proyecto y corra las pruebas automáticamente, sin depender de que alguien las corra a mano antes de subir un cambio.

### Qué se agregó

- **`.github/workflows/ci.yml`** — workflow de GitHub Actions que se dispara en cada `push`. Configura el SDK de **.NET 10**, restaura las dependencias (`dotnet restore`), compila la solución (`dotnet build`) y corre las pruebas (`dotnet test`) sobre `CitasApp.slnx`.
- **`CitasApp.xUnit`** — nuevo proyecto de pruebas agregado a la solución, referenciando las capas `CitasApp.Domain` y `CitasApp.Application` para poder probar la lógica de negocio de forma aislada.
- **Pruebas unitarias** sobre al menos 3 clases del proyecto, siguiendo el patrón **Arrange-Act-Assert** con el atributo `[Fact]` de xUnit, incluyendo repositorios *fake* en memoria (implementaciones reales de las interfaces del Domain, sin librerías de mocks) en lugar de acceder a los archivos JSON reales.
- **Evidencia de detección de fallos real**: se incluyó intencionalmente una prueba que expone un caso no controlado en `CalculadoraService.Dividir` (división entre cero, que en `double` no lanza excepción sino que regresa `Infinity`). Esa prueba se subió primero en rojo ❌ para demostrar que el pipeline sí detecta fallas, y luego se corrigió el método para que valide el caso y la prueba pasó a verde ✅.

### Cómo verlo

- Pestaña **Actions** del repositorio → historial de ejecuciones del workflow `CI`.
- Checks de verificación en los Pull Requests de esta rama.
